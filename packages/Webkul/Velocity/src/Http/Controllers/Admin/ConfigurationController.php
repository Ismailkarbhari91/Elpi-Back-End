...
class ConfigurationController extends Controller
{
    ...
    public function renderMetaData()
    {
        ...
        return view($this->_config['view'], [
            'metaData' => $velocityMetaData,'datas'=>$datas
        ]);
    }
    ...
    public function storeMetaData($id)
    {
        ...
        // 
        $about = json_decode($velocityMetaData->about_images, true);

        $params['about_images'] = [];

        if (isset($params['imagesabout'])) {
            foreach ($params['imagesabout'] as $index => $images) {
                $params['about_images'][$index] =  $this->uploadAboutusImages($images, $index, $about);
            }

            if ($about) {
                foreach ($about as $key => $image_array) {
                    if (! isset($params['imagesabout'][$key])) {
                        foreach ($about[$key] as $image) {
                            Storage::delete($image);
                        }
                    }
                }
    ...
    public function uploadAdvertisementImages($data, $index, $advertisement)
    {
        ...
        return $saveImage;
             ...
             // 
             public function uploadAboutusImages($data, $index, $about)
             {
                 $saveImage = [];
                 $dir = 'velocity/images';
                 ...
                 foreach ($data as $imageId => $image) {
                     if ($image != "") {
                         $file = 'imagesabout.' . $index . '.' . $imageId;
                         ...
                         if (Str::contains($imageId, 'image_')) {
                             ...
                             if (
                                 isset($about[$index][$imageId])
                                 && $about[$index][$imageId]
                                 && !request()->hasFile($file)
                             ) {
                                 $saveImage[$imageId] = $about[$index][$imageId];
         
                                 unset($about[$index][$imageId]);
                             }
                             ...
                             if (
                                 ...
                                 Storage::delete($about[$index][$imageId]);
                         ...
                         if ($saveData) {
                             if (isset($about[$index][$subIndex])) {
                                 $saveImage[$subIndex] = $about[$index][$subIndex];
         
                                 if (sizeof($about[$index]) == 1) {
                                     unset($about[$index]);
                                 } else {
                                     unset($about[$index][$subIndex]);
                                 }
                             }
                         } else {
                             if (! isset($about[$index][$subIndex])) {
                 ...
                 if (
                     ...
                     foreach ($about[$index] as $imageId) {
                         Storage::delete($imageId);
                     }
                 }
